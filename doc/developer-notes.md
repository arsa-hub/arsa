...
- [Development guidelines](#development-guidelines)
    - [General Arsa Core](#general-arsa-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Arsa Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/arsa-hub/arsa/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/arsad -printtoconsole
```
...
Arsa Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Arsa Core, where everyone uses
...
pay attention to for reviewers of Arsa Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Arsa Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Arsa Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `arsa-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `arsa-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Arsa Core, the wallet was always
