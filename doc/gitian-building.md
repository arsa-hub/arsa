...
*Setup instructions for a Gitian build of Arsa Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Arsa
...
- [Building Arsa Core](#building-arsa-core)
...
Clone the git repositories for Arsa Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/arsa-hub/arsa
...
in the Arsa Core repository under 'Fetch and create inputs' to install sources which require
...
To build Arsa Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Arsa Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/arsa/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/arsa-hub/arsa
...
./bin/gbuild --commit arsa=${COMMIT} --url arsa=${URL} ../arsa/contrib/gitian-descriptors/gitian-osx.yml
```
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../arsa/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/arsa
SIGPATH=/some/root/path/arsa-detached-sigs

./bin/gbuild --url arsa=${BTCPATH},signature=${SIGPATH} ../arsa/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/arsa-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/arsa-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/arsa-osx-build.assert
```
...
[arsa/gitian.sigs](https://github.com/arsa-hub/gitian.sigs/) repository, or if that's not possible to create a pull
request.
