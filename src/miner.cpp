...
void static ArsaMiner(const CChainParams& chainparams)
{
    LogPrintf("ArsaMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("arsa-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("ArsaMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("ArsaMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("ArsaMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("ArsaMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateArsas(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&ArsaMiner, boost::cref(chainparams)));
    }
    return(numCores);
}
