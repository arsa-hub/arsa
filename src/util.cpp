...
const char * const BITCOIN_CONF_FILENAME = "arsa.conf";
const char * const BITCOIN_PID_FILENAME = "arsad.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "arsad -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ArsaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ArsaCore
    // Mac: ~/Library/Application Support/ArsaCore
    // Unix: ~/.arsacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ArsaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ArsaCore";
#else
    // Unix
    return pathRet / ".arsacore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty arsa.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Arsa Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Arsa Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Arsa Core developers";
    }
