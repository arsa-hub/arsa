...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/ArsaParams"
else
    PARAMS_DIR="$HOME/.arsa-params"
fi
...
DOWNLOAD_URL="https://github.com/arsa-hub/arsa/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with ARGY_DISABLE_SOMETHING=1
ARGY_DISABLE_WGET="${ARGY_DISABLE_WGET:-}"
#ARGY_DISABLE_IPFS="${ARGY_DISABLE_IPFS:-}"
ARGY_DISABLE_CURL="${ARGY_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$ARGY_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$ARGY_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$ARGY_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Arsa parameters!
...
main() {
    ...
    cat <<EOF
ARGY - fetch-params.sh

This script will fetch the Arsa SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Arsa SNARK parameters. Note that it is
