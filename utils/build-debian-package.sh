...
PACKAGE_VERSION=$($SRC_PATH/src/arsad --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/arsad $DEB_BIN
cp $SRC_PATH/src/arsa-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/arsa-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/arsad.1 $DEB_MAN
cp $SRC_DOC/man/arsa-cli.1 $DEB_MAN
cp $SRC_DOC/man/arsa-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/arsad.bash-completion $DEB_CMP/arsad
cp $SRC_PATH/contrib/arsa-cli.bash-completion $DEB_CMP/arsa-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/arsad.1
gzip --best -n $DEB_MAN/arsa-cli.1
gzip --best -n $DEB_MAN/arsa-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/arsad $DEB_BIN/arsa-cli
